...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if ((string(argv[i]) == "-C" || string(argv[i]) == "--config_path") && i + 1 < argc)
        {
            ...
            cout << "Stokes_Solver.cpp: Configuration path obtained from parser options: " << config_path << endl;
            break;
        }
    }
    ...
    string mesh_output_path = output_dir + globalVars.sim_type + "_mesh.mesh";
    ...
    Mesh *mesh = new Mesh(mesh_dir + mesh_file_name);
    ...
    cout << "Number of unknowns in velocity: " << fespace_u->GetTrueVSize() << endl;
    cout << "Number of unknowns in pressure: " << fespace_p->GetTrueVSize() << endl;
    cout << "Number of unknowns in total: " << fespace_u->GetTrueVSize() + fespace_p->GetTrueVSize() << endl;
